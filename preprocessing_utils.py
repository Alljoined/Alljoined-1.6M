...
#     epochs: List[mne.Epochs],         # List of MNE Epochs, one per session
#     stim_order: pd.DataFrame,         # Metadata dataframe describing the expected stimuli
#     verbose: bool = False,            # If True, print how many triggers were dropped per session
# ) -> np.ndarray:
...
#     kept_indices: List[int] = []  # Final list of all matched trial indices across sessions

#     for sess in SESSIONS:  # <--- PROBLEM: 'SESSIONS' is not defined anywhere in the function or passed in
#         # Get events array from the current session (shape: [n_events, 3])
#         ev = epochs[sess - 1].events

#         # Create reverse lookup from event code to description
#         code_to_desc = {v: k for k, v in epochs[sess - 1].event_id.items()}

#         # Extract the 4th comma-separated component of the event label and convert to int
#         trigger_vals = np.array(
#             [int(code_to_desc[e].split(",")[3]) for e in ev[:, 2]]
#         )

#         # Subset stim_order for the current session
#         session_df = stim_order[(stim_order["session"] == sess)].copy()

#         # Extract numerical image indices from filenames (assuming filenames end in something like '12345.jpg')
#         session_df["image_index"] = (
#             session_df["image_path"].str[-9:-4].astype(int)
#         )
#         img_idx_arr = session_df["image_index"].to_numpy()

#         indices: List[int] = []  # List of matched indices in the current session
#         ptr = 0  # Pointer in the image index array

#         for val in trigger_vals:  # For each trigger in EEG
#             # Move pointer until we find a match or reach the end
#             while ptr < len(img_idx_arr) and img_idx_arr[ptr] != val:
#                 ptr += 1

#             # If we reach the end without matching, the sequence is invalid
...
#             # Keep the index of the matched trial
#             indices.append(session_df.index[ptr])
#             ptr += 1  # Move to next index for subsequent matches

#         # Optionally print warning about dropped trials
